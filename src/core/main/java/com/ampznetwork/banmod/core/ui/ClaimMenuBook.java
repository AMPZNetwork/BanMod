package com.ampznetwork.banmod.core.ui;

import com.ampznetwork.banmod.api.BanMod;
import com.ampznetwork.banmod.api.game.Flag;
import com.ampznetwork.banmod.api.model.adp.BookAdapter;
import com.ampznetwork.banmod.api.model.mini.PlayerRelation;
...
public class ClaimMenuBook implements BookAdapter {
    ...
    private Component[] toc() {
        ...
        return new Component[]{
                ...
                text("%s by %s\n".formatted(bestName.matches(RegExpUtil.UUID4_PATTERN)
                        ? "Unnamed " + rcTitle
                        : bestName, banMod.getPlayerAdapter().getName(claimOwner))),
                text("\n"),
                text("\n"),
                text("2 - ")
    ...
    private Component[] details() {
        ...
        var compOwner = text("Owner: %s".formatted(claimOwner == null
                ? "none" : banMod.getPlayerAdapter().getName(claimOwner)));
            ...
            List<@NotNull TextComponent> entries(PlayerRelation type) {
                ...
                return (switch (type) {
                    ...
                    default -> throw new IllegalStateException("Unexpected value: " + type);
                        ...
                        .map(id -> {
                            var name = banMod.getPlayerAdapter().getName(id);
                            return text()
                                    .append(text("[-]")
                                            .color(RED)
