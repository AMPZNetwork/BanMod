...
public class Infraction extends DbObject {
    public static final               EntityType<Infraction, Builder<Infraction, ?>> TYPE         = Polyfill.uncheckedCast(
            new EntityType<>(Infraction::builder,
                    null,
                    Infraction.class,
                    Infraction.Builder.class));
    public static final               Instant                                        TOO_EARLY    = Instant.EPOCH.plus(
            Duration.ofDays(2));
    public static final               Predicate<Infraction>                          IS_IN_EFFECT = i -> !i.getPunishment()
            .isInherentlyTemporary() && (i.getRevoker() == null && (i.getExpires() == null || i.getExpires()
            .isAfter(now()) || i.getExpires().isBefore(TOO_EARLY))) /* fix for a conversion bug */;
    public static                     Comparator<Infraction>                         BY_SEVERITY  = Comparator.<Infraction>comparingInt(
            i -> i.getPunishment().ordinal()).reversed();
    public static                     Comparator<Infraction>                         BY_NEWEST    = Comparator.<Infraction>comparingLong(
            i -> i.timestamp.toEpochMilli()).reversed();
    public static                     Comparator<Infraction>                         BY_SHORTEST  = Comparator.<Infraction>comparingLong(
            i -> i.expires == null ? Long.MIN_VALUE : i.expires.toEpochMilli()).reversed();
    @NotNull @OneToOne                Player                                         player;
    @NotNull @ManyToOne               PunishmentCategory                             category;
    @NotNull                          Punishment                                     punishment;
    @NotNull @lombok.Builder.Default  Instant                                        timestamp    = now();
    @Nullable @lombok.Builder.Default Instant                                        expires      = null;
    @Nullable @lombok.Builder.Default String                                         reason       = null;
    @ManyToOne                        Player                                         issuer       = null;
    @ManyToOne                        Player                                         revoker      = null;
    @Nullable @lombok.Builder.Default Instant                                        revokedAt    = null;

    public @Nullable String getReason() {
        return Optional.ofNullable(reason)
                .or(() -> Optional.of(category).map(DefaultReason::getDefaultReason).filter(not(String::isBlank)))
                .or(() -> Optional.of(punishment).map(DefaultReason::getDefaultReason).filter(not(String::isBlank)))
